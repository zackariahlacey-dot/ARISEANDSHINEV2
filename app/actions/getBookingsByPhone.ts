...
export async function getBookingsByPhone(phone: string): Promise<{
  ...
  const { data } = await supabase
    .from("bookings")
    .select("id, booking_date, booking_time, status, total_price, service_name, vehicle_year, vehicle_make, vehicle_model, vehicle_size, service_address, customer_phone, user_id")
    .or(conditions)
