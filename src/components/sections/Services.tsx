...
import { Sparkles, Shield, Crown, ArrowRight } from "lucide-react";

const packages = [
  { id: "01", name: "Interior", price: "150", icon: <Sparkles className="w-5 h-5 text-white/40" />, details: ["Steam Sanitization", "Deep Extraction", "Leather Care", "Odor Removal"] },
  { id: "02", name: "Exterior", price: "125", icon: <Shield className="w-5 h-5 text-white/40" />, details: ["Deionized Wash", "Clay Bar Prep", "Hand Waxing", "Wheel Precision"] },
  { id: "03", name: "Elite Full", price: "250", icon: <Crown className="w-5 h-5 text-[#fbbf24]" />, details: ["Total Restoration", "Hyper Gloss Wax", "Stain Removal", "Headlight Clarity"], featured: true },
];
  ...
  return (
    <Section id="services" spacing="none" className="py-24 md:py-48 bg-[#050505] relative border-y border-white/5">
      <div className="max-w-7xl mx-auto px-6 md:px-12 mb-24 text-center">
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="text-5xl md:text-9xl font-black text-white uppercase tracking-tighter leading-none mb-10 italic">
          Signature <span className="text-white/10">Precision</span>
        </motion.h2>
      ...
      >
        {packages.map((pkg, index) => (
          ...
          >
            <div className={cn(
              "h-full p-10 md:p-14 flex flex-col relative glass-panel rounded-[40px] transition-all duration-700",
              ...
              <div className="mb-10">
                <h3 className="text-3xl md:text-4xl font-black text-white mb-4 uppercase tracking-tight">{pkg.name}</h3>
                <div className="flex items-baseline gap-1">
                  <span className="text-white/20 text-xl font-medium">$</span>
                  <span className="text-5xl md:text-7xl font-black text-white group-hover:text-[#fbbf24] transition-colors duration-700 tracking-tighter">{pkg.price}</span>
                </div>
              </div>
              ...
              <div className="space-y-4 mb-12 flex-grow">
                {pkg.details.map(detail => (
                  <div key={detail} className="flex items-center gap-4 group/item">
                    ...
                    <span className="text-xs md:text-sm font-bold uppercase tracking-widest text-white/40 group-hover/item:text-white/80 transition-colors italic">{detail}</span>
                  </div>
                ))}
              </div>
              ...
              <PrismButton 
                ...
                className="w-full py-6 text-[10px] font-black uppercase tracking-[0.3em] rounded-full"
